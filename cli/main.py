...
@click.group()
@click.version_option(version='0.4.0', prog_name='sayu')
def cli():
...
@cli.command()
@click.option('--start', is_flag=True, help='Start background daemon')
@click.option('--stop', is_flag=True, help='Stop background daemon')
@click.option('--restart', is_flag=True, help='Restart background daemon')
def daemon(start, stop, restart):
    """Manage Sayu background daemon"""
    try:
        repo_root = CliUtils.require_git_repo()
        
        # Import daemon module
        from daemon.sayud import SayuDaemon
        ...
        if start:
            ...
            if daemon.is_running():
                ...
                print(f"📁 Repository: {repo_root}")
                print(f"⏱️  Uptime: {daemon._get_uptime()}")
            else:
                print("⏹️  Daemon not running")
                print("💡 Start with: sayu daemon --start")
